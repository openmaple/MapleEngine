...
from mdata import m_asm
from mdata import m_mirmpl
from mlldb import m_config_lldb as m_debug
from mlldb import m_symbol
...
class MapleMirbinInfoIndex():
    ...
    def get_asm_mirbin_item(self, asm, mirbin_name):
        ...
        if not mirbin_name in self.mirbin_info_cache[asm]:
            ...
            m_debug.dbg_print("Maple label", mirbin_name, "not found in mirbin info cache ")
            return None
...
class MapleFileCache():
    ...
    def show_general_fullpath(self):
        mdb_print("mlldb_rdata.general_fullpath cache ==== ")
        mdb_print(str(self.general_fullpath))
    ...
    def show_src_file_lines(self):
        mdb_print("mlldb_rdata.src_file_lines cache ==== ")
        mdb_print(str(self.src_file_lines))
    ...
    def show_asm_block_lines(self):
        mdb_print("mdb_rdata.asm_block_lines cache ==== ")
        mdb_print(str(self.asm_block_lines))
    ...
    def show_mir_block_lines(self):
        mdb_print("mlldb_rdata.mir_block_lines cache ==== ")
        mdb_print(str(self.mir_block_lines))

class MapleLLDBRuntimeData():
    """
    class MapleLLDBRuntimeData is an object at runtime, create and manage
    ...
    def update_mdb_runtime_data(self):
        ...
        if not self.mirmpl_info_cache_has_key(mirmpl_path):
            if m_debug.Debug: mdb_print("create Maple mir.mpl cache using file: " + mirmpl_path)
            self.create_mirmpl_info_cache(mirmpl_path)

        if not self.mirbin_info_cache_has_key(asm_path):
            if m_debug.Debug: mdb_print("create Maple symbol cache using file: " + asm_path)
        ...
        if not def_file_path in self.class_def.def_paths:
            if m_debug.Debug: mdb_print("create def cache using file: " + def_file_path)
            self.create_class_def(def_file_path)


mlldb_rdata =  MapleLLDBRuntimeData()
...
def get_stack_frame_data(frame):
    ...
    asm_line_tuple = mlldb_rdata.get_one_label_mirbin_info_cache(asm_path, func_header_name)
    if not asm_line_tuple:
        if m_debug.Debug: m_debug.dbg_print("no asm_line_tuple found", asm_path, "for", func_header_name)
        return None
    if m_debug.Debug:
    ...
    mirmpl_line_tuple = mlldb_rdata.get_one_label_mirmpl_info_cache(mirmpl_path, func_header_name[:-12])
