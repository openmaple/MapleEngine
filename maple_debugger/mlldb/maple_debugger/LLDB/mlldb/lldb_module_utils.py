...
class MDumpLineTables:
    command_name = "mdump-line-tables"
...
class MDumpFiles:
    command_name = "mdump-files"
    short_description = "Dumps full paths to compile unit files and optionally all line table files."
    usage = "usage: %prog [options] MODULE1 [MODULE2 ...]"
    description = '''This class adds a mdump-files command to the LLDB interpreter.
...
def __lldb_init_module(debugger, dict):
    ...
    debugger.HandleCommand(
        'command script add -c %s.MDumpLineTables %s' % (__name__,
                                                        MDumpLineTables.command_name))
    debugger.HandleCommand(
        'command script add -c %s.MDumpFiles %s' % (__name__, MDumpFiles.command_name))
    print('The "%s" and "%s" commands have been installed.' % (MDumpLineTables.command_name,
                                                               MDumpFiles.command_name))
