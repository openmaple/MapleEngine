...
# global maple breakpoint table shared with static and dynamic mbps
mbp_table = {}
...
def is_mbp_dync_existed():
    """ determine where a Maple breakpoint exists """
    blist = gdb.breakpoints()
    for b in blist:
        if m_debug.Debug: m_debug.dbg_print("b.type=", b.type, "b.location=", b.location, "b.thread=", b.thread, "b.enabled=", b.enabled,\
                        "b.is_valid()=", b.is_valid())
        if 'maple::InvokeInterpretMethod' in b.location and b.is_valid():
            return True, b
    return False, None
...
def update_maple_invoke_dync_bp(buf, op_enable = True):
    """
        update maple::maple_invoke_method breakpoint
        when number of enabled maple breakpoints is 0, disable maple::maple_invoke_method
        when number of enabled maple breakpoints changes from 0 to non-0, enable maple::maple_invoke_method
        ...
        params:
          ...
          op_enable = False, to disable the maple::maple_invoke_method
    """
    match_pattern = "in maple::InvokeInterpretMethod(maple::DynMFunction&) at"
    buf = buf.split('\n')
    for line in buf:
        if match_pattern in line:
            cmd = 'disable ' if op_enable is False else 'enable '
...
def get_maple_invoke_dync_bp_stop_addr(buf):
    """
    get Maple breakpoint address and its coresponding information
...
class MapleBreakpoint(gdb.Breakpoint):
    ...
    def stop(self):
        ...
        if not match_pattern in mbp_table:
            return False

        # Maple symbol for the breakpoint is disabled
        table_match_pattern = mbp_table[match_pattern]
    ...
    def display_mbp_table(self):
        global mbp_table
        for mspec in mbp_table:
            buf = 'mapleBreakpoint object mbp_table[' + str(mspec) + ']: ' + str(mbp_table[mspec])
            gdb_print(buf)
    ...
    def get_pending_addr_symbol_list(self):
        global mbp_table
        return [ k for k, v in mbp_table.items() if not v['hex_addr'] ]
    ...
    def add_known_addr_symbol_into_addr_sym_table(self, symbol):
        ...
        """
        global mbp_table
        if not symbol in mbp_table:
            return

        if mbp_table[symbol]['hex_addr'] and mbp_table[symbol]['address'] != 0:
    ...
    def update_pending_addr_symbol(self, symbol):
        ...
        if not addr:
            return
        mbp_table[symbol]['hex_addr'] = addr
        mbp_table[symbol]['address'] = int(addr,16)
    ...
    def get_enabled_mbp_number(self):
        global mbp_table
        return len([ k for k, v in mbp_table.items() if not v['disabled'] ])
    ...
    def clearup_mbp_symbol_related_data(self):
        global mbp_table
        for mspec in mbp_table:
            mbp_table[mspec]['address'] = 0
            mbp_table[mspec]['hex_addr'] = None

        self.mbp_addr_sym_table = {}
        self.load_objfiles = 0


class MapleBreakpointDync(gdb.Breakpoint):
    """
    MapleBreakpoint class creates an instance of Maple breakpoint using gdb python API,
    and provides Maple breakpoint stop control logic.
    """
    ...
    def __init__(self, mtype=gdb.BP_BREAKPOINT, mqualified=True):
        ...
        3, Disables unnecessary plt breakpoint.
        """
        super().__init__('maple::InvokeInterpretMethod')
        buf = m_util.gdb_exec_to_str("info b")
        disable_maple_invoke_dync_bp_plt(buf)
        #self.mbp_table = {} # the symbols here are NOT mirbin_info symbols
        self.bp_addr, self.bp_info = get_maple_invoke_dync_bp_stop_addr(buf)
    ...
    def stop(self):
        ...
        # plt will be not disable. So we disable it
        buf = m_util.gdb_exec_to_str("info b")
        if not is_maple_invoke_dync_bp_plt_disabled(buf):
            disable_maple_invoke_dync_bp_plt(buf)
            return False
        ...
        # NOTE!!! here we are getting the current stack's mir_header's address
        args_addr = m_symbol.get_dynamic_symbol_addr_by_current_frame()
        if not args_addr or not args_addr in self.mbp_addr_sym_table:
            return False
        ...
        # update the Maple gdb runtime metadata store.
        #m_datastore.mgdb_rdata.update_gdb_runtime_data()
        m_datastore.mgdb_rdata.update_gdb_runtime_data_dync()
        # update the Maple frame change count
        m_datastore.mgdb_rdata.update_frame_change_counter()
        return True
    ...
    def update_pending_addr_symbol(self, symbol):
        ...
        # this is a workaround of the case where symbol is "main" while it should be
        # like js_main or py_main 
        if symbol != 'main':
            addr = m_symbol.get_symbol_address(symbol)
            if not addr:
                return
    ...
    def update_mbp_dync(self):
        ...
        if mbp_num == 0:
            # disable this breakpoint
            update_maple_invoke_dync_bp(buf, False)
        else:
            # enable this breakpoint
            update_maple_invoke_dync_bp(buf, True)
