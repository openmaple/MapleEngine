...
def generate_trace_stack(filename):
    ...
    + + ==> m_util.py:31, gdb_exec_to_string, @(m_breakpoint.py:140), {arg#1, info b}
    - - <-- m_util.py:32, gdb_exec_to_string, 66us, ret: Num...\n
    ...
    + + + ==> m_util.py:31, gdb_exec_to_string, @(m_symbol.py:68), {arg#1, p/x *(long long*)&mir_header}
    - - - <-- m_util.py:32, gdb_exec_to_string, 57us, ret: $105 = 0x7fffea8cae28\n
        ...
        if buf[0] == "+":
            for idx in range(len(buf)):
                if buf[idx] == "==>":
                    ...
                    fstack.append(buf[idx + 1] + " " + func_name)
                    etime.append(0)
                    gdbarg = ' '.join(buf[idx + 4:]) if func_name.startswith("gdb_exec") else ""
                    break
        if buf[0] == "-":
            for idx in range(len(buf)):
                if buf[idx] == "<--":
                    ...
                    if len(gdbarg) > 0 and buf[idx + 2].startswith("gdb_exec"):
                        print(gdbarg)
