...
TESTROOT="$MAPLE_DEBUGGER_ROOT"/testcases/JavaScript/
...
while read d; do
    testcase=$(basename "$d")
    echo -n Running JavaScript test case "$testcase"...
    ...
    cp -a .mdbinit "$MAPLE_BUILD_ROOT/$TESTCASE"/*.js "$app"/ || exit $LINENO
    grep -v "^echo " .mdbinit | sed -e '/^python/,/^end/s/^/@/' -e 's/^[^@].*/echo (gdb) & \\n\n&/' -e 's/^@//' > "$app"/.mdbinit
    cd "$app" || exit $LINENO
    "$MAPLE_DEBUGGER_TOOLS"/debugger-trace-js.sh >& log.txt &
    pid=$!
    maxtry=$TIMEOUT
